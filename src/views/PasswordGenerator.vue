...
export default defineComponent({
    ...
    emits: ['copyToClipboard', 'messageShow'],
    watch: {
        passLength() {
            this.generateNewPass();
        ...
        passChars() {
            this.generateNewPass();
            localStorage.setItem('passChars', this.passChars.toString());
        },
    },
    methods: {
        restoreSession() {
            let passLength = localStorage.getItem('passLength');
            let passChars = localStorage.getItem('passChars');
            if (passLength) {
                this.passLength = parseInt(passLength);
            }
            if (passChars) {
                this.passChars = passChars.split(',');
            }
        },
        generateNewPass(byClick = false) {
            ...
            if (byClick) {
                this.$emit('messageShow', 'okay', 'New password was generated');
            }
        },
        showPass() {
...
<template>
    <div class="content">
        ...
        <div>
            ...
            <multi-button icon="refresh" @click="generateNewPass(true)" />
            <multi-button
                icon="eye"
                @mousedown="showPass()"
                @mouseup="hidePass()"
