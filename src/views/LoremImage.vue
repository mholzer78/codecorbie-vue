...
export default defineComponent({
    ...
    beforeMount() {
        ...
        this.directDownload =
            paramWidth !== '' &&
            paramHeight !== '' &&
            !isNaN(parseFloat(paramWidth)) &&
            !isNaN(parseFloat(paramHeight));
    ...
    methods: {
        ...
        downloadMe() {
            const canvasForDownload = document.querySelector(
                '#canvasForDownload'
            )! as HTMLCanvasElement;
            this.fillCanvas(
...
<template>
    <div class="content">
        <div>
            In the preview below, only the ratio is updated accordingly, the
            size of the preview is limited.
        </div>
        ...
        <div>
            For direct download try entering https://{{
                hostname
            }}/lorem-image/{{ width }}/{{ height }}/{{ colorHexUrl }}
        </div>
        <div>
            <canvas
